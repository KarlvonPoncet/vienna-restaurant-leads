...
(() => {
  ...
  async function loadDetail(id) {
    try {
      ...
      const toggle = document.createElement("button"); toggle.id = "draft-preview-toggle"; toggle.type = "button"; text(toggle, "Show plain-text/source"); toggle.addEventListener("click", () => { const source = $("draft-source"); source.hidden = !source.hidden; frame.hidden = !source.hidden; toggle.textContent = source.hidden ? "Show plain-text/source" : "Show rendered HTML"; }); previewBox.appendChild(toggle);
